...
**Status (MVP plan):** M0–M8 **Done** (TASK-004–012). Ship/merge TASK-012 to land M8 on `develop`.

---

## M0 — Design lock + monorepo scaffold

**Status:** Done (TASK-004)
...
| Deliverable                     | Notes                                                                                     |
| ------------------------------- | ----------------------------------------------------------------------------------------- |
| Gold fixtures                   | 정답 문장 없음, 통과 형태/skip — `apps/api/src/write/fixtures/` + `gold-fixtures.spec.ts` |
| Leak / length / language checks | `physical-laws` (`internal_leak` / `over_length` / `wrong_language`)                      |
| Backup note                     | `docs/project/11-ops-backup.md`                                                           |
| DESIGN/LAYOUT/UX 회귀           | `docs/project/UI-DRIFT-CHECKLIST.md`                                                      |
